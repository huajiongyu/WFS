...
//绑定元素
$(function () {
    var _proviceOptionsHTML = "";
    $.each(provice, function () {
        _proviceOptionsHTML += "<option value='" + this.name + "'>" + this.name + '</option>';
    });
    $("#BankProvice").append(_proviceOptionsHTML);//添加开户省份的选项


    var _city = $("#BankCity"),_city1 = $("#BankCity2");
    //当开户省份变化时，城市联动变化
    $("#BankProvice").on('change', function () {
        var _citiesHtml = "";
        ...
        $.each(PObj[0].city, function (index, item) {
            _citiesHtml += "<option value='" + item.name + "'>" + item.name + "</options>";
        });
        _city.append(_citiesHtml);
    ...
    _city.on('change', function () {
        ...
        var _c = this.value;
        var CObj = $.grep(PObj[0].city, function (item) { return item.name === _c; });
        $.each(CObj[0].districtAndCounty, function (index, item) {
            _citiesHtml += "<option value='" + item + "'>" + item + "</options>";
        });
        _city1.append(_citiesHtml);
    });

    $("#BankProvice").val($("#BankProvice").data('value')).trigger('change');
    _city.val(_city.data('value')).trigger('change');
    _city1.val(_city1.data('value'));
